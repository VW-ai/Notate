...
#### RefineStyle
```typescript
type RefineStyle = 'professional' | 'polite' | 'friendly' | 'shorter' | 'longer'
```
...
| content | string | ✓ | 文本内容 |
| file | File? | - | 文件数据（image/file 类型时） |
| habitId | string? | - | 指定执行的 Habit ID |

**输出：**
| 字段 | 类型 | 说明 |
|------|------|------|
| capture | Capture | 创建的 Capture |
| evolutionHint | EvolutionHint? | 演化提示（如检测到相似内容） |
